...
class CloneVariationalAutoEncoder(nn.Module):
    """
    Given (s, a) --> Reconstruct a
    """
    def __init__(
        ...
        super(CloneVariationalAutoEncoder, self).__init__()

        net_arch = [1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024]
        layer = create_mlp(state_dim, action_dim, net_arch=net_arch)
        self.layer = th.nn.Sequential(*layer)
        self.max_action = max_action

    def forward(self, state: th.Tensor) -> Tuple:
        """
        state: [batch size, state dimension]
        action: [batch size, action dimension]
...
class CloneMIN(OffPolicyAlgorithm):
    """
    Minimalist approach for offline reinforcement learning.
    The algorithm based on TD3.
    ...
    def __init__(
        ...
        if isinstance(self.alpha, str) and self.alpha.startswith("auto"):
            state_dim = get_flattened_obs_dim(self.observation_space)
            action_dim = get_action_dim(self.action_space)
            self.vae = CloneVariationalAutoEncoder(
                state_dim,
                action_dim,
                100,
                self.action_space.high[0],
    ...
    def _setup_model(self) -> None:
        super(CloneMIN, self)._setup_model()
        self._create_aliases()
    ...
    def get_action_variance(self) -> th.Tensor:
        ...
        tile_observations = th.repeat_interleave(observations, 10, 0)
        noise = tile_observations.clone().data.normal_(0, 0.05)
        tile_observations = tile_observations + noise

        tile_ae_action = self.vae(tile_observations)
        tile_ae_action = tile_ae_action.reshape(batch_size, 10, -1)  # [batch_size, 10, action_dim]
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        # Start: train autoencoder
        reconstructed_action = self.vae(replay_data.observations)
        autoencoder_loss = th.mean((reconstructed_action - replay_data.actions) ** 2)
    ...
    def learn(
            ...
            eval_freq: int = -1,
            n_eval_episodes: int = 5,
            tb_log_name: str = "CloneMIN",
            eval_log_path: Optional[str] = None,
            reset_num_timesteps: bool = True,
    ) -> OffPolicyAlgorithm:

        return super(CloneMIN, self).learn(
            total_timesteps=total_timesteps,
            callback=callback,
            log_interval=log_interval,
            eval_env=eval_env,
    ...
    def _excluded_save_params(self) -> List[str]:
        return super(CloneMIN, self)._excluded_save_params() + ["actor", "critic", "actor_target", "critic_target"]
...
if __name__ == "__main__":
    ...
    file_name = f"{args.env_name}-{args.degree}"
    if args.clone:
        file_name += "-clone"
    ...
    if not args.clone:
        ...
        model = CloneMIN("MlpPolicy", env=env, learning_starts=0, without_exploration=True, alpha="auto", gradient_steps=1)

    for epoch in range(100):
        model.learn(1_0000, reset_num_timesteps=False)
        z = model.get_action_variance()
        print("Clone:", args.clone)
        print("DEGREE", args.degree)
        print("VARIANCE", th.mean(z) * 100)
        print("\n\n")
        with open(f"../GQEdata/quality/{file_name}", "a") as f:
            f.write(f"{(z * 1000).mean().item()}\n")
