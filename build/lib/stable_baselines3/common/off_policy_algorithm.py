...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def learn(
        ...
        # if self.buffer_load_interval is None:
        #     self.buffer_load_interval = 1_000_000 // total_timesteps
        # """
        # 아래 if문을 넣어주지 않으면, base_class.py의 408번줄 부근 if self.ep_info_buffer로 안들어가져서
        # buffer가 제대로 init되지 않는다. 사실 ep_info_buffer 자체가 필요없긴 함.
        # ep_info_buffer: episode하나 끝났을 때, 해당 episode의 reward등의 정보만 담고있는 dict. 학습에 영향 X.
        # """
        # if self.without_exploration:
        #     if self.offline_round_step % self.buffer_load_interval == 0:
        #         self.reload_buffer = True
        #         self.ep_info_buffer = None      # Episode 하나 끝날 때 마다 그 episode의 reward, std dev등을 담고있는 buffer. 학습 자체에는 필요 없고 logging에만 필요함.
    ...
    # def load_buffer(self, num_buffers: int = 1) -> None:
    #     # print(self.data_dir)        # /Pong
    #     if self.d4rlenv:
    #         self._load_d4rl_env()

        # if DEBUG:
        #     self.load_replay_buffer(self.data_dir)
        #     # degree = degree
        #     # print("degree", degree)
        #     # self.load_replay_buffer(f"../testbuffer/LunarLander-v2-{degree}")
        # else:
        #     agent = np.random.choice(np.arange(1, 6), num_buffers, replace=False)
        #     suffices = np.random.choice(50, num_buffers, replace=False)
        #     data_dirs = [self.data_dir + f"/{a}/replay_logs/" for a in agent]
        #     with futures.ThreadPoolExecutor(max_workers=16) as thread_pool_executor:
        #         _ = [thread_pool_executor.submit(self._load_buffer, data_dir, suffix)
        #              for data_dir, suffix in zip(data_dirs, suffices)]
        #     assert self.replay_buffer.full

    def _load_buffer(self, data_dir: str, suffix: int) -> None:
        """
        :param data_dir: "~~~/replay_logs/" 형태여야 함.
        :param suffix: Suffix of the dataset. (Between 1 to 50).
        """
        print("********** Load Buffer **********")
        print(f"{data_dir}{suffix}")
        print("********** Load Buffer **********")
        elements = ["observation", "action", "reward", "terminal"]
        buffer = {}
        for elem in elements:
            filename = f'{data_dir}{STORE_FILENAME_PREFIX}{elem}_ckpt.{suffix}.gz'
            with open(filename, 'rb') as f:
                with gzip.GzipFile(fileobj=f) as infile:
                    buffer[elem] = np.load(infile)

        buffer["observations"] = buffer["observation"]  # Fit to stable baselines3
        del buffer["observation"]
        buffer["actions"] = buffer["action"]
        del buffer["action"]
        buffer["rewards"] = buffer["reward"]
        del buffer["reward"]
        buffer["dones"] = buffer["terminal"]
        del buffer["terminal"]

        buffer["next_observations"] = np.zeros_like(buffer["observations"])
        buffer["next_observations"][:-1] = buffer["observations"][1:]

        # This may be missing, but this may not effect to training.
        buffer["next_observations"][-1] = buffer["observations"][0]
        ...
        dones = buffer["dones"]
        infos = [None for _ in range(buffer_size)]

        for obs, next_obs, action, reward, done, info in zip(observations, next_observations, actions, rewards, dones, infos):
    ...
    def _dump_logs(self) -> None:
        ...
        if self.without_exploration:
            ...
            self.logger.record("train/n_critics", self.policy_kwargs["n_critics"])
        ...
        if len(self.offline_rewards) > 0 and len(self.offline_normalized_rewards) > 0:
            self.logger.record("train/rewards_mean", np.mean(self.offline_rewards))
            self.logger.record("train/normalized_rewards_mean", np.mean(self.offline_normalized_rewards))
