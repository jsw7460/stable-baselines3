...
from typing import Dict, Iterable, Optional, Tuple, Union, Sequence, Callable
...
from stable_baselines3.common.type_aliases import (
    GymEnv, Schedule, TensorDict, TrainFreq, TrainFrequencyUnit, ReplayBufferSamples
)
...
def quantile_huber_loss(predicted_quantiles: th.Tensor,
                        target_quantiles: th.Tensor,
                        midpoint_quantiles: th.Tensor,
                        kappa: float = 1.0
                        ) -> th.Tensor:
    """
    :param predicted_quantiles: A predicted quantiles. [batch_size, n_quantiles]
    :param target_quantiles: A target quantiles. [batch_size, n_quantiles]
    :param midpoint_quantiles: midpoint quantiles in QRDQN, or beta_taus in IQN.
    :param kappa: a boundary of quantile huber loss, notated by kappa in the paper
    :return: quantile huber loss, explained in QR-DQN paper.
    """
    # pairwise_delta = target_quantiles.unsqueeze(-2) - predicted_quantiles.unsqueeze(-1)
    # abs_pairwise_delta = th.abs(pairwise_delta)
    # huber_loss = th.where(abs_pairwise_delta>1, abs_pairwise_delta - 0.5, pairwise_delta ** 2 * 0.5)
    # loss = th.abs(midpoint_quantiles - (pairwise_delta.detach() < 0).float()) * huber_loss
    #
    # return loss.mean()

    # Parsing the size
    batch_size, n_quantiles = predicted_quantiles.size()

    # Expand the quantiles across the different axis
    target_quantiles = target_quantiles.unsqueeze(1).expand(batch_size, n_quantiles, n_quantiles)
    predicted_quantiles = predicted_quantiles.unsqueeze(2).expand(batch_size, n_quantiles, n_quantiles)
    # target_quantiles = target_quantiles.unsqueeze(-2)
    # predicted_quantiles = predicted_quantiles.unsqueeze(-1)
    """
    Expand the midpoint quantiles along the axis same as target quantiles. 
    Here, 1, because target quantiles expanded over its COLUMN.
    Finally, tau_hat has the size [n_quantiles, n_quantiles]
    """
    if midpoint_quantiles.dim() == 1:
        midpoint_quantiles = midpoint_quantiles.unsqueeze(-1).expand(n_quantiles, n_quantiles)
    elif midpoint_quantiles.dim() == 2:
        midpoint_quantiles = midpoint_quantiles.unsqueeze(-1).expand(batch_size, n_quantiles, n_quantiles)
    else:
        pass
    # midpoint_quantiles = midpoint_quantiles.unsqueeze(0).unsqueeze(2)
    """
    1. Compute the Huber loss. 
    To do this, we compute the absolute value of difference of target and prediction. 
    This is a boundary condition of Huber loss.
    """
    difference_of_quantiles = target_quantiles - predicted_quantiles
    huber_loss = th.where(
        th.abs(difference_of_quantiles) <= kappa,
        difference_of_quantiles ** 2 * 0.5,
        kappa * (th.abs(difference_of_quantiles) - kappa * 0.5)
    )

    """
    2. Finally, compute the Quantile Huber loss.
    This is an equation (10) of the QR-DQN paper.
    To do this, we need to compute where the difference of quantiles is negative.
    """
    final_quantile_huber_loss = th.where(
        difference_of_quantiles < 0,
        th.abs((midpoint_quantiles - 1)) * huber_loss / kappa,
        midpoint_quantiles * huber_loss / kappa
    ).sum()

    return final_quantile_huber_loss / batch_size
...
class D4rlReplayBuffer(object):
    """
    Convert the d4rl dataset to a replay buffer to be matched with stable baselines3.
    """
    ...
    def _to_tensor(self) -> None:
        self.observations = th.tensor(self.dataset["observations"]).to(self.device)
        self.actions = th.tensor(self.dataset["actions"]).to(self.device)
        self.next_observations = th.tensor(self.dataset["next_observations"]).to(self.device)
        self.rewards = th.tensor(self.dataset["rewards"]).unsqueeze(1).to(self.device)
        self.dones = th.tensor(self.dataset["rewards"]).unsqueeze(1).to(self.device)
        self.buffer_size = self.observations.size(0)

    def sample(self, batch_size: int) -> ReplayBufferSamples:
        assert self.buffer_size >= batch_size, "batch size should be smaller than buffer size"
