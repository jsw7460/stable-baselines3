...
class CQL(OffPolicyAlgorithm):
    ...
    def _get_conservative_loss(self, replay_data, current_q_values: th.Tensor) -> th.Tensor:
        ...
        # generate random action on [-1, 1]
        # [batch_size, num_randoms, num_actions]
        # random_actions \
        #     = 2 * th.rand(size=(batch_size, self.num_randoms, num_actions)).to(self.device) - 1
        # random_actions \
        #     = th.FloatTensor(batch_size, self.num_randoms, num_actions).uniform_(-1, 1).to(self.device)
        ...
        # q1_random_action_q_values, q2_random_action_q_values \
        #     = self.critic.cql_forward(observations, random_actions)
        # Compute (s, a_i) where a_i follows the policy.
        # q1_current_action_q_values, q2_current_action_q_values \
        #     = self.critic.cql_forward(observations, current_actions)
        ...
        # min_q_loss = th.mean(min_q_losses)
        # print(min_q_loss)

        # conservative_loss = th.sum(th.tensor(min_q_loss))
        # next_observations = th.repeat_interleave(replay_data.next_observations, repeats=self.num_randoms, dim=0)
        # next_actions, next_actions_log_probs = self.actor.action_log_prob(next_observations)
        # next_actions = next_actions.reshape(batch_size, self.num_randoms, -1)
        # next_actions_log_probs = next_actions_log_probs.reshape(batch_size, self.num_randoms, )
        #
        # q1_next_action_q_values, q2_next_action_q_values \
        #     = self.critic.cql_forward(next_observations, next_actions)
        #
        # q1_cat = th.cat(
        #     [q1_random_action_q_values - random_density,
        #      q1_next_action_q_values - next_actions_log_probs.detach(),
        #      q1_current_action_q_values - current_log_probs.detach()]
        # )
        # q2_cat = th.cat(
        #     [q2_random_action_q_values - random_density,
        #      q2_next_action_q_values - next_actions_log_probs.detach(),
        #      q2_current_action_q_values - current_log_probs.detach()]
        # )


        # min_q1_loss = th.logsumexp(q1_cat, dim=1).mean() - current_q_values[0].mean()
        # min_q2_loss = th.logsumexp(q2_cat, dim=1).mean() - current_q_values[1].mean()

        # Compute final conservative loss. Since we have two q networks, so average them
        # conservative_loss = (min_q1_loss + min_q2_loss) / 2
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            # critic_loss = original_critic_loss
