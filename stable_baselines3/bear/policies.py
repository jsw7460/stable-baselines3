...
class VariationalAutoEncoder(nn.Module):
    ...
    def forward(self, state: th.Tensor, action: th.Tensor) -> Tuple:
        ...
        reconstructed_action = self.decode(state, latent_vector, self.device)
        ...
        if latent_vec is None:
            ...
            with th.no_grad():
                latent_vec = th.randn((batch_size, self.latent_dim), device=device, dtype=th.float32)\
                    .clamp_(-self.max_action, +self.max_action)
...
class BearPolicy(BasePolicy):
    ...
    def __init__(
        ...
        super(BearPolicy, self).__init__(
            observation_space,
            action_space,
