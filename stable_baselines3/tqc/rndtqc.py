...
from stable_baselines3.tqc.tqc import quantile_huber_loss
from stable_baselines3.common.torch_layers import create_mlp

class RNDTQC(OffPolicyAlgorithm):
    """
    ...
    def _setup_model(self) -> None:
        super(RNDTQC, self)._setup_model()
        ...
        # NOTE ---------- Add for RNDTQC: Build a predictor and target network -------------
        predictor_arch = create_mlp(
            input_dim=self.observation_space.shape[0] + self.action_space.shape[0],
            output_dim=100,
            net_arch=[2, 2],
            activation_fn=th.nn.ReLU,
        )
        self.rnd_predictor = th.nn.Sequential(*predictor_arch).to(self.device)
        self.rnd_optimizer = th.optim.Adam(self.rnd_predictor.parameters(), lr=1e-4)

        target_arch = create_mlp(
            input_dim=self.observation_space.shape[0] + self.action_space.shape[0],
            output_dim=100,
            net_arch=[1024, 1024, 1024],
            activation_fn=th.nn.ReLU,
        )
        self.rnd_target = th.nn.Sequential(*target_arch).to(self.device)
        # Target network initialized randomly, and not learnable.
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        # NOTE Start ----------------- RND loss compute & update ------------------
        rnd_input = th.cat([replay_data.observations, replay_data.actions], dim=1).float()
        rnd_predict = self.rnd_predictor(rnd_input)
        rnd_target = self.rnd_target(rnd_input)

        rnd_error = (rnd_predict - rnd_target) ** 2  # [batch_size, rnd_output_dim]
        rnd_loss = th.mean(rnd_error)
        ...
        # NOTE End ----------------- RND loss compute & update --------------------
        with th.no_grad():
        ...
        with th.no_grad():
            ...
            rnd_input = th.cat([replay_data.next_observations, next_actions], dim=1).float()
            rnd_predict = self.rnd_predictor(rnd_input)
            rnd_target = self.rnd_target(rnd_input)
            rnd_error = (rnd_predict - rnd_target) ** 2 * 10 # [batch_size, rnd_output_dim]
            quantiles_variance = th.mean(rnd_error, dim=1, keepdim=True)      # [batch_size]
            n_target_quantiles \
        ...
        if self.use_policy_penalty:
            with th.no_grad():
                ...
                p_trunc_current_quantiles \
                    = p_current_quantiles[:, :, self.top_quantiles_to_drop_per_net : -self.top_quantiles_to_drop_per_net]
    ...
    def learn(
        ...
        tb_log_name: str = "RNDTQC",
        eval_log_path: Optional[str] = None,
        reset_num_timesteps: bool = True,
    ) -> OffPolicyAlgorithm:

        return super(RNDTQC, self).learn(
    ...
    def _excluded_save_params(self) -> List[str]:
        # Exclude aliases
        return super(RNDTQC, self)._excluded_save_params() + ["actor", "critic", "critic_target"]
