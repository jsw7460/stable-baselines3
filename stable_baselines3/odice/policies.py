...
def get_kl_divergence(
    ...
    """
    # Compute v(s')
    v_s = donsker(replay_data.observations)
    ...
    if policy_no_grad:
        t_mean, t_log_std = t_mean.detach(), t_log_std.detach()
        b_mean, b_log_std = b_mean.detach(), b_log_std.detach()
    ...
    t_var_prod = th.prod(t_var, dim=1)
    b_var_prod = th.prod(b_var, dim=1)

    # Compute (x-mean).T @ inverse(covariance) @ (x-mean)
    # Since covariance is diagonal, so inverse is just reciprocal of diagonal matrix
    t_mean_dist = th.sum((1 / t_var) * ((replay_data.actions - t_mean) ** 2), dim=1)
    b_mean_dist = th.sum((1 / b_var) * ((replay_data.actions - b_mean) ** 2), dim=1)
    ...
    bellman_v_s = gamma * v_s * rnd
    donsker_loss = th.log(th.mean(th.exp(v_s - bellman_v_s))) - (1 - gamma) * th.mean(v_s)
