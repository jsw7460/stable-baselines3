...
class QuantileNetwork(BasePolicy):
    ...
    def __init__(self,
        ...
        quantile_net = create_mlp(self.features_dim + action_dim, action_dim * self.n_quantiles, self.net_arch, self.activation_fn)
    ...
    def forward(self, obs: th.Tensor) -> th.Tensor:
        ...
        :return: Estimated quantiles for each action.
        """
    ...
    def _predict(self, obs: th.Tensor, deterministic: bool = True) -> th.Tensor:
        q_values = self.forward(obs).mean(dim=1)
...
class CnnPolicy(QRDQNPolicy):
    """
    Policy class for QRDQN when using images as input
    """
    def __init__(
            self,
            observation_space: gym.spaces.Space,
