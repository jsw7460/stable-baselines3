...
class VariationalAutoEncoder(nn.Module):
    ...
    def forward(self, state: th.Tensor, action: th.Tensor) -> Tuple:
        ...
        log_std.clamp_(-4, 15)      # For the stability of numerical computation
        std = th.exp(log_std)

        latent_vector = mean + std * th.randn_like(std)     # [batch size, latent_dim]
    ...
    def decode(self, state: th.Tensor, latent_vec: Optional[th.Tensor] = None, device: th.device = "cuda:0") -> th.Tensor:
        ...
        if latent_vec is None:      # Used to Sample action from "next" states. See algorithm in the BCQ paper.
            ...
            with th.no_grad():
                latent_vec = th.randn((batch_size, self.latent_dim), device=device, dtype=th.float32).clamp_(-self.max_action, +self.max_action)
...
class NextStateAutoEncoder(nn.Module):
    """
    Given (s, a, s') --> Reconstruct s'
    """
    def __init__(
        self,
        state_dim,
        ...
        super(NextStateAutoEncoder, self).__init__()
        encoder_arch = [300, 300]
        encoder_net = create_mlp(state_dim + action_dim + state_dim, 750, encoder_arch)       # 750: Same with original BCQ code
        self.encoder = nn.Sequential(*encoder_net)
        self.encoder = nn.Sequential(
            th.nn.Linear(state_dim + action_dim + state_dim, 300),
            th.nn.ReLU(inplace=True),
            th.nn.Linear(300, 750),
        )
        ...
        decoder_arch = [200, 200]
        decoder_net = create_mlp(state_dim + action_dim + latent_dim, state_dim, decoder_arch, squash_output=False)
        self.decoder = nn.Sequential(*decoder_net)

        self.latent_dim = latent_dim
        self.max_state = max_state

    def forward(self, state: th.Tensor, action: th.Tensor, next_state: th.Tensor) -> Tuple:
        mean, log_std = self.encode(state, action, next_state)
        std = th.exp(log_std)

        latent_vector = mean + std * th.randn_like(std)     # [batch_size, latent_dim]
        reconstructed_next_state = self.decode(state, action, latent_vector, mean.device)

        return reconstructed_next_state, mean, log_std

    def encode(self, state: th.Tensor, action: th.Tensor, next_state: th.Tensor) -> Tuple:
        """
        Return: mean, log_std
        """
        encoder_input = th.cat([state, action, next_state], dim=1).float()
    ...
    def decode(
        ...
        if latent_vec is None:
            ...
            with th.no_grad():
                ...
                latent_vec.clamp_(-self.max_state, +self.max_state)
        decoder_input = th.cat([state, action, latent_vec], dim=1).float()
        next_state = self.decoder(decoder_input)

        return next_state

        # FIXME: 여기 좀 수정할 수 있을 것 같음. 우선 대부분의 환경에서는 self.max_state가 inf라서 이대로 하면 안됨
        # FIXME: 예를들어, state space를 [0, 1]로 normalize한다던지 그런걸 할 수 있어 보임
        # return self.max_state * next_state
...
class SACPolicy(BasePolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        # self.critic.optimizer = self.optimizer_class(critic_parameters, lr=2e-5)
