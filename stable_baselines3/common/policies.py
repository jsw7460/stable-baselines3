...
class ContinuousCritic(BaseModel):
    ...
    def __init__(
        ...
        for idx in range(n_critics):
            q_net = create_mlp(features_dim + action_dim, 1, net_arch, activation_fn, dropout=dropout)
    ...
    def cql_forward(self, obs: th.Tensor, actions: th.Tensor) -> Tuple:
        """
        Predict the Q-value using ith network
        This code is only for the cql.

        obs: [batch_size * num_rands, observation_dim]
        actions: [batch_size, num_rands, num_actions]
        # TODO: obs, actions 사이즈 다르다.
        """
        batch_size, num_random, _ = actions.size()
        with th.set_grad_enabled(not self.share_features_extractor):
            features = self.extract_features(obs).reshape(batch_size, num_random, -1)

        qvalue_input = th.cat([features, actions], dim=-1)

        return tuple(q_net(qvalue_input).squeeze() for q_net in self.q_networks)

    def repeated_forward(self, obs: th.Tensor, actions: th.Tensor, batch_size) -> Tuple:
        """
        Forward the q-network with repeated time
        obs: [batch size x repeats, state dim]
        actions: [batch size x repeats, action dim]
        Repeat하는 액션을 밖에서 만들어서 주는 경우가 있음. E.g, BCQ, CQL. 그래서 repeat된 상태로 들어옴.
        예를들어, noise같은 걸 bcq에서 action에 추가한 뒤에 여기로 넘어옴.

        --> Input of q-network = [batch_size * repeat, state dim + action dim]
        --> Output of q-network = [batch_size, repeat, 1]
        """

        repeat = actions.size(0) // batch_size

        with th.set_grad_enabled(not self.share_features_extractor):
            features = self.extract_features(obs)
        qnet_input = th.cat([features, actions], dim=1)

        return tuple(q_net(qnet_input).squeeze().reshape(batch_size, repeat, -1) for q_net in self.q_networks)



    def edac_forward(self, obs: th.Tensor, actions: th.Tensor) -> Tuple:
        """
        Added for EDAC. Due to computation of the gradient.
        :param obs: [num_critics, batch_size, observation_dim]
        :param actions: [num_critics, batch_size, action_dim]
        :return Tuple, whose length = num_critics.
        """
        num_qs, batch_size, obs_dim = obs.size()
        obs = obs.view(-1, obs_dim)
        with th.set_grad_enabled(not self.share_features_extractor):
            features = self.extract_features(obs).view(num_qs, batch_size, -1)  # [num_critics, batch_size, feature_dim]

        qvalue_input = th.cat([features, actions], dim=-1)
        return tuple(q_net(qvalue_input[i]) for (i, q_net) in zip(range(num_qs), self.q_networks))
