...
class BaseAlgorithm(ABC):
    ...
    def _setup_learn(
        ...
        if self.action_noise is not None and not without_exploration:
            self.action_noise.reset()

        if reset_num_timesteps:     # For offline setting, always false
            self.num_timesteps = 0
            self._episode_num = 0
        else:
        ...
        if not without_exploration:     # Only for online RL.
