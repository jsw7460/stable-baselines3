...
class NavigationEnvDefault(gym.Env, EzPickle):
    def __init__(self, task_args, max_obs_range=3,  max_speed=2, initial_speed=2, **kwargs):
        ...
        if not task_args['OBSTACLE_POSITIONS']:
            self.obstacles = None
        else:
    ...
    def reset(self):
        ...
        self.drawlist = [self.obs_range_plt, self.drone, self.goal] + self.walls
        if self.obstacles is not None:
            self.drawlist += self.obstacles.dynamic_bodies
...
class NavigationEnvRandomInit(NavigationEnvDefault):
    @property
    def drone_start_pos(self):
...
class NavigationEnvAccRandomInit(NavigationEnvAcc):
    @property
    def drone_start_pos(self):
        return denormalize_position(np.random.random(2) * 0.8 + 0.1, W ,H)
...
class NavigationEnvAccLidarObs(NavigationEnvAcc):
    def __init__(self, task_args, max_obs_range=3, max_speed=2, initial_speed=1, **kwargs):
        ...
        self.observation_meta_data_keys = ['position', 'lidar', 'energy', 'velocity']
...
class NavigationEnvAccLidarObsRandomInit(NavigationEnvAccLidarObs):
    @property
    def drone_start_pos(self):
        return denormalize_position(np.random.random(2) * 0.8 + 0.1, W ,H)
...
class StationaryNavigation(NonStationaryNavigation):
    def __init__(self, task_args, max_obs_range=3, max_speed=2, initial_speed=1, task_loss=0, **kwargs):
        super().__init__(task_args, max_obs_range, max_speed, initial_speed, **kwargs)
        self.observation_meta_data = {
            'position': gym.spaces.Box(np.array([0, 0]), np.array([W, H]), dtype=np.float32),
            'distance': gym.spaces.Box(np.array([0]), np.array([np.sqrt(W ** 2 + H ** 2)]), dtype=np.float32),
            ...
            'energy': gym.spaces.Box(low=0, high=1, shape=(1,)),
            'velocity': gym.spaces.Box(np.array([-1, -1]), np.array([1, 1]), dtype=np.float32),
            'goal': gym.spaces.Box(low=0, high=1, shape=(2,)),
        }

        self.task_loss = task_args['task_loss']
        self.observation_meta_data_keys = ['position', 'lidar', 'energy', 'distance', 'velocity', 'goal']
    ...
    def moving_goal(self):
        if np.random.random() > self.uncertainty:
            ...
            self.theta = np.random.random() * np.pi * 2
        goal = denormalize_position((np.array([np.cos(self.theta), np.sin(self.theta)]) * (0.374 + np.random.random() * 0.1)) + np.array([0.5, 0.5]), W, H)
        self.task_args['Goal'] = goal
        self.goal.position.Set(goal[0], goal[1])
        self.prev_distance = self.distance
        return